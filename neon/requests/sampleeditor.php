...
if($isEditor){
	if($action == 'save'){
		if($inquiryManager->editSample($_POST)) $status = 'close';
	}
	elseif($action == 'savenew'){
		$inquiryManager->setid($id);
		if($inquiryManager->addSample($_POST)) $status = 'close';
		else $errStr = $inquiryManager->getErrorStr();
	}
	elseif($action == 'deleteSample'){
		if($inquiryManager->deleteSample($samplePK)) $status = 'close';
	}
}
?>
<html>
...
<div id="popup-innertext">
	...
	if($isEditor){
		...
		$sampleArr = array();
		if($samplePK) $sampleArr = $inquiryManager->getSampleArr($samplePK);
		?>
