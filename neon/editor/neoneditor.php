				...
				<summary style="cursor:pointer; font-weight:bold; font-size:16px;">
					...
					<h4><b><u>Determinations</u></b></h4>
						<p style="margin-left:10%; margin-right:10%; font-size:15px"> This tool allows you to add determinations by adding a new determination record and updating the associated fields within the primary occurrence record.
							...
							The tidInterpreted value will be populated by the scientific name provided if there is one and only one taxon with that scientific name in the portal's taxonomic hierarchy. If there is 0 or more than 1 matching tid, tidInterpreted will be null in the records.
							If family and authorship fields are not mapped and a tid match is found, these fields will be populated with values in the NEON taxonomy tables. Determinations in the NEON portal should be as complete as possible, including references, qualifiers, etc.  If the determination being added is the current determination, put 1 in the field <strong>isCurrent</strong>. If isCurrent is not mapped or not set as 1, the new determination will not become the active determination. Check the box below the field mappings to propagate the new determination to all other occurrences with a derivedFromSameIndividual association defined within the portal.
							If the identification should be protected, include securityStatus with a value of 1 and securityStatusReason with a value of "Locked - NEON redaction list."
