...
class IgsnManager{
	...
	public function getIgsnTaskReport(){
		...
		$sql = 'SELECT c.collid, CONCAT_WS("-",c.institutioncode,c.collectioncode) as collcode, c.collectionname, count(o.occid) as cnt
			...
			WHERE c.institutionCode IN ("NEON","ASU") AND c.collid NOT IN(44,74,78,79,80,82,83,95,97,81,85,93,96,84,115) AND o.occurrenceId IS NULL AND s.sampleReceived = 1
			GROUP BY c.collid';
	...
	public function getIgsnUpdateReport(){
		...
		$sql = 'SELECT 
				...
				INNER JOIN NeonSample s ON o.occid = s.occid
				WHERE c.institutionCode IN ("ASU","NEON")
	...
	public function setNullNeonIdentifiers(){
		$sql = 'UPDATE omoccurrences o INNER JOIN NeonSample s ON o.occid = s.occid '.
			'SET o.catalognumber = SUBSTRING_INDEX(o.occurrenceID, "10.58052/", -1) '.
			'WHERE o.occurrenceID LIKE "https://doi.org/10.58052/NEON%" AND o.catalognumber IS NULL';
		$this->conn->query($sql);
	}
