	...
	<body>
		...
		<div id="innertext">
			...
			<div id = "heroimage-div">
				<img src="<?php echo $CLIENT_ROOT . '/images/card-images/20240820-by betancourt - sophia holds calosoma scrutator-29.jpg'; ?>" style="max-width:100%" alt="ASU biorepository" loading="lazy">
			</div>
			<p>NEON has been collecting specimens as part of its sampling program since 2012. In 2018, NEON began construction of the NEON Biorepository – a facility intended to archive most specimens curated by the NEON program - at the Arizona State University Biocollections in Tempe, AZ. The NEON Biorepository publishes records for every archived sample and specimen and makes these samples available for loan.</p>
			...
			<p>Every sample is identified in the NEON database with a unique primary identifier. All samples are designated a <b>Sample Tag (sampleID)</b> and/or <b>Barcode (sampleCode)</b> upon collection and a <b>Catalog Number (archiveGuid)</b> upon physical accession at the Biorepository. Each sample is also designated a <b>Sample Class (sampleClass)</b>, which roughly equates to sample type. <a href="https://data.neonscience.org/data-products/explore">NEON data products</a> that include sampling will often record sampleIDs in their data tables. Within each data product download, an accompanying table that describes the products' variables will help you identify the sampleID variable names and the data table(s) in which they appear. All samples in the NEON Biorepository can be searched by sampleID, catalog number or barcode through the <a href="../neon/search/index.php">Sample Search</a> page.</p>
			<h3>Sample Tags</h3>
			<p>A Sample Tag (sampleID) is the identifier for the sample and contains as much information as is required to differentiate specimens within the same sample class; these identifiers may contain information about the sample encoded in the label, including the site or plot of collection, the date or time of collection, the taxon or sex (as applicable), or a unique number (e.g., tag number on a small mammal). Sample tags are auto-generated at time of collection within the field collection app, Fulcrum. As the observatory has moved into operations, sample tags have been supplemented and/or replaced by barcodes.</p>
			<img src="<?php echo $CLIENT_ROOT . '/images/embed/mos_sample_tag.jpg'; ?>" style="" alt="Photo of a pinned mosquito" loading="lazy">
			<h3>Barcodes</h3>
			<p>Barcode (sampleCode) labels were first used on NEON samples in 2017 on a select number of NEON specimens and expanded to most NEON sample types in subsequent years. NEON uses cryo-compatible barcode labels for specimens stored in -20 to -196 ˚C and non-cryo barcode labels for the remaining samples. The non-cryo labels are robust to exposure in solvents (particularly ethanol and water) and are optimized to hold up to field conditions. Within NEON systems, barcodes cannot be duplicated – as in, two separate/different samples cannot be affiliated with the same barcode. This enables NEON to leverage barcode tracking for duplicate checking in the database.</p>
			<img src="<?php echo $CLIENT_ROOT . '/images/embed/barcode_2.png'; ?>" style="" alt="Type II barcode" loading="lazy">
			<h3>Catalog Numbers</h3>
			<p>Upon arrival and check-in at the NEON Biorepository, samples are assigned a Catalog Number (archiveGUID). The Catalog Number is shorter, uniform, and globally unique across all accessioned samples. It ensures consistency, as sampleIDs can vary in structure and length based on sample type or collection details. Using a uniform catalog number streamlines sample tracking and referencing across systems, simplifying specimen management and ensuring accurate records as samples are accessed or loaned for research. These identifiers are also registered with <a href="https://www.geosamples.org/" target="_blank">SESAR</a>, which assigns the catalog numbers an IGSN ID through DataCite. As a result, each sample is globally unique and web-resolvable with a DOI. (e.g., <a href="https://doi.org/10.58052/NEON01I8P" target="_blank">https://doi.org/10.58052/NEON01I8P</a>)</p>
			<img src="<?php echo $CLIENT_ROOT . '/images/embed/small_mammal.jpg'; ?>" style="" alt="Jar containing a shrew with a label" loading="lazy">			
			<h3>Sample Classes</h3>
			<p>Sample Classes (sampleClass) roughly equate to types of samples, such that carabid specimens mounted on pins (sample class: bet_IDandpinning_in.individualID) have a different class than mounted mosquito specimens (mos_identification_in.individualIDList). The first part of the sample class name identifies the data table that is input into NEON's database, e.g., bet = beetle, IDandpinning = identification and pinning activity, in = ingest. The second part of the sample class name identifies the specific field in the table that holds identifiers. New sample classes are often assigned as an originating field-collected sample goes through various processing steps, so not all sample classes correspond to samples archived at the Biorepository.</p>
